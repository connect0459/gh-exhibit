...
// local filesystem, per docs/SPEC.md's on-disk layout.
package persistence
...
// local filesystem, per docs/SPEC.md's on-disk layout. Unexported so
// callers depend only on the repositories.AttachmentWriter interface, not
...
// docs/SPEC.md's issues/{repo}/{number}/assets/{filename} and
