...
func NewIssueRef(owner, repo string, number int) (IssueRef, error) {
	...
	if number <= 0 {
		return IssueRef{}, errors.New("repositories: issue ref number must be positive")
	}
...
func validateOwner(owner string) error {
	if owner == "" {
		return errors.New("repositories: issue ref owner must not be empty")
	}
	if len(owner) > maxOwnerLength {
		return fmt.Errorf("repositories: issue ref owner must be at most %d characters, got %d", maxOwnerLength, len(owner))
	}
	if !ownerPattern.MatchString(owner) {
		return fmt.Errorf("repositories: issue ref owner %q is not a valid GitHub username", owner)
	}
	return nil
}
...
func validateRepoName(repo string) error {
	if repo == "" {
		return errors.New("repositories: issue ref repo must not be empty")
	}
	if repo == "." || repo == ".." {
		return fmt.Errorf("repositories: issue ref repo must not be %q", repo)
	}
	if len(repo) > maxRepoLength {
		return fmt.Errorf("repositories: issue ref repo must be at most %d characters, got %d", maxRepoLength, len(repo))
	}
	if !repoPattern.MatchString(repo) {
		return fmt.Errorf("repositories: issue ref repo %q is not a valid GitHub repository name", repo)
	}
	return nil
}
