...
func NewInlineContext(path string, line *int, diffHunk string, outdated bool) (InlineContext, error) {
	if path == "" {
		return InlineContext{}, errors.New("valueobjects: inline context path must not be empty")
	}
	if line != nil && *line <= 0 {
		return InlineContext{}, errors.New("valueobjects: inline context line must be positive when present")
	}
	if line == nil && outdated {
		return InlineContext{}, errors.New("valueobjects: inline context cannot be outdated without a line")
	}
