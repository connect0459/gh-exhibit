	...
	for _, raw := range rawReviewComments {
		...
		if markSeen(seenCommentIDs, id) {
			skipped = append(skipped, SkipNote{
				Reason: fmt.Sprintf("timeline: duplicate review comment id %d", id),
				Raw:    raw,
			})
			continue
		}
...
func buildReviewComment(raw json.RawMessage) (valueobjects.InlineReviewComment, int64, int64, error) {
	...
	if err := json.Unmarshal(raw, &w); err != nil {
		return valueobjects.InlineReviewComment{}, 0, 0, fmt.Errorf("timeline: unmarshal review comment: %w", err)
	}

	attribution, err := valueobjects.NewAttribution(w.User.resolvedLogin(), w.CreatedAt, w.HTMLURL)
	if err != nil {
		return valueobjects.InlineReviewComment{}, 0, 0, fmt.Errorf("timeline: review comment attribution: %w", err)
	}
	line, outdated := w.resolvedLine()
	ctx, err := valueobjects.NewInlineContext(w.Path, line, w.DiffHunk, outdated)
	if err != nil {
		return valueobjects.InlineReviewComment{}, 0, 0, fmt.Errorf("timeline: review comment context: %w", err)
	}
